...
%! close_or_far_display(+Board).
% Uses the best close_or_far restriction method to solve the desired puzzle Board
% (@see close_or_far_geral and @see restrict), and displays the solved board with
% @see display_board
close_or_far_display(Board) :-
	close_or_far_geral(Board, restrict),
	display_board(Board).

%! generate(+Side, +Unique, +Name).
...
% The solved board is generated by @see close_or_far_geral with restrict after 
...
% The Board is displayed in the terminal using @see display_board 
% and saved in the generated_boards.pl file using @see generate_predicate with the chosen Name
% @param Name: Name of the generated predicate on generated_boards.pl
generate(Side, Unique, Name):-
    board_length(Side, FinishedBoard),
	close_or_far_geral(FinishedBoard, restrict),
    build_board(Side, FinishedBoard, Board),
    unique(Unique, FinishedBoard, Board),
	generate_predicate(Board, Name),
	display_board(Board), nl.

%! generate_stats(+Unique, +Labeling, +Side, -Flag, +Name, +Method).
...
generate_stats(Unique, Labeling, Side, Flag, Name, Method):-
	write(Side), write(','),
    board_length(Side, FinishedBoard),
	close_or_far_stats(FinishedBoard, Method, Labeling, Flag),
	generate_on_flag(Side, Unique, FinishedBoard, _, Flag, Name).

% generate_on_flag(+Side, +Unique, +FinishedBoard, -Board, +Flag).
% Determines if there was a timeout during labeling, since this means
% this is not fit to build a new board.
generate_on_flag(_, _, _, _, time_out, _).
generate_on_flag(Side, Unique, FinishedBoard, Board, _, Name):-
    build_board(Side, FinishedBoard, Board),
    unique(Unique, FinishedBoard, Board),
	generate_predicate(Board, Name).
...
% The Cols variable indicates where each hint is by specifing the column index in each row of the list
...
unique(unique2, _, Board):-
	length(List, 1),
	findall(_, close_or_far_geral(Board, restrict), List).
unique(Unique, _, _):-
	Unique \= unique1,
	Unique \= unique2.
