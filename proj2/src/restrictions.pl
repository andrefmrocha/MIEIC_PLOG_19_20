...
% Cols is a list with the row index of each column element
...
% The Cols return indicates where each hint is by specifing the row index in each column of the list
...
% Row and Column constraint are guarantee by the all_distinct and the data structure (list) respectively
% Uses @see constrain not to ensure the diagonal part of the problem, but to ensure that the chosen element
% is not an empty space (0). Board is the solved puzzle
ntowers(Cols, Board) :-
	length(Board, N),
	length(Cols, N),
	domain(Cols, 1, N),
...
% Ensures that the chosen element to each column is not an empty space in the original solution
constrain([], []).
...
% TODO: ver colunas vs rows do Cols vs FinishedBoard
generate_board([], [], []).
